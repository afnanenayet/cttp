...
bind_addr(int port);
static bool
unbind_addr(int sock_fd);
static char*
ip_addr_str(struct sockaddr_in* addr);
static char*
network_req_str(int sock_fd);
void*
cttp_resp_worker(void* args);
...
cttp_resp_worker(void* args)
{
  ...
  char* full_fp_str =
    calloc(strlen(root) + strlen(base_path) + 2, sizeof(char));
...
cttp_server_run(int port, const char* root)
{
  ...
  while (server.stat == 0) {
    ...
    if (rc != 0 ||
      ...
      free(thr_args);
      fprintf(stderr,
              "Failed to create worker thread to dispatch HTTP "
              "response\n");
    }
  }
